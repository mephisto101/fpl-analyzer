...
DIFF_LIGHT_TEXT = {3}  # difficulties where black text is used
...
# 5. HELPER FUNCTIONS
# ==========================================
def get_upcoming_fixtures(team_id, fixtures, data, num_gws=FIXTURE_LOOKAHEAD_GWS):
    upcoming = []
    try:
...
def get_display_df(df, cols):
    return df[cols].rename(columns={c: COLUMN_LABELS.get(c, c) for c in cols})

def fetch_squad_picks(manager_id, gw_id):
    """Fetch squad picks for a manager in a given gameweek. Returns list of picks or None."""
...
# ==========================================
tabs = st.tabs(["My Team", "Global Scout", "Ticker", "Player VS", "Rivals", "Transfers"])
...
with tabs[0]:
    if not my_squad.empty:
        ...
        st.dataframe(
            get_display_df(my_squad, ['web_name', 'team_name', 'pos', 'price', 'form', 'total_points', 'ict_index']),
...
with tabs[1]:
    ...
    st.dataframe(
        get_display_df(
            scout_df.sort_values('total_points', ascending=False),
            ['web_name', 'team_name', 'pos', 'price', 'total_points', 'form', 'ict_index'],
        ),
        use_container_width=True,
        hide_index=True,
    )
...
# --- TAB 2: TICKER ---
with tabs[2]:
    st.header("5-Week Fixture Ticker")
    ...
    st.dataframe(
        df_tick.style.map(style_ticker, subset=df_tick.columns[1:]),
        use_container_width=True,
        hide_index=True,
    )
...
with tabs[5]:
    ...
    if not my_squad.empty:
        ...
        if sorted_squad.empty:
            st.info("No squad data available.")
        else:
            worst = sorted_squad.iloc[0]
            ...
            with tc2:
                ...
                # Budget: worst player price + buffer to find affordable upgrades
